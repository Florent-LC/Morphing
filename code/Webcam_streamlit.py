...
class WebCamVideo:
    ...
    def detect_face_live(
        self, image1: st.delta_generator.DeltaGenerator, detect: bool
    ):
        while self.video_capture.isOpened():
            ...
            image1.image(
                image, use_column_width=True, caption="Transformed input"
            )

        self.video_capture.release()
        cv2.destroyAllWindows()

    def morphing_realtime(
        self, image1: st.delta_generator.DeltaGenerator, img_model: Image
    ):
        while self.video_capture.isOpened():
            retrieve, frame = self.video_capture.read()
