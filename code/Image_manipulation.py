...
class Image:
    ...
    def rotation(self, angle: float):
        ...
        rotate_matrix = cv2.getRotationMatrix2D(
            center=self.center, angle=angle, scale=1
        )
        rotated_image = cv2.warpAffine(
            src=self.img, M=rotate_matrix, dsize=(self.width, self.height)
        )
    ...
    def write_text(
        self, text: str, position: Tuple[int, int], fontscale: float = 0.5
    ):
        ...
        if not (boxes is None):
            for box, prob in zip(boxes, probs):
                if prob > threshold:
                    ...
                    self.write_text(
                        f"{prob_truncate}%", (int((a + c) / 2), d + 10)
                    )
    ...
    def set_landmarks(self):
        ...
        if not (self.box is None):
            ...
            self.landmarks_list = [
                (point.x, point.y) for point in landmarks.parts()
            ]
            ...
            if self.model:
                ...
                self.landmarks_dictionnary = {
                    coordinate: index
                    for index, coordinate in enumerate(self.landmarks_list)
                }
    ...
    def extract_face(self):
        ...
        if not (self.box is None):
            ...
            # in the other case, we apply this operation for the complementary of the mask
                ...
                self.face = cv2.bitwise_and(
                    self.img, self.img, mask=self.inverted_mask
                )
    ...
    def set_Delaunay_Triangulation(self):
        ...
        if not (self.box is None):
            ...
            self.triangles = np.array(
                self.subdiv.getTriangleList(), dtype=np.int32
            ).reshape((-1, 3, 2))
            ...
            indexes_triangles = [
                [
                    self.landmarks_dictionnary[tuple(e[0])],
                    self.landmarks_dictionnary[tuple(e[1])],
                    self.landmarks_dictionnary[tuple(e[2])],
                ]
                for e in self.triangles
            ]
            self.indexes_triangles = np.array(
                indexes_triangles, dtype=np.uint8
            )

    def morphing(
        img_model: "Image", img_applied: "Image", debug: bool = False
    ):
        ...
        # of the first face
        img_applied.triangles = img_applied.landmarks_array[
            img_model.indexes_triangles
        ]
        ...
        for i in range(len(img_model.triangles)):
            ...
            warped_triangle = cv2.warpAffine(
                cropped_triangle1, matrix_warping, (w, h)
            )
            ...
            warped_triangle = cv2.bitwise_and(
                warped_triangle, warped_triangle, mask=cropped_triangle_mask2
            )
            ...
            img_morphing_rect_area_gray = cv2.cvtColor(
                img_morphing_rect_area, cv2.COLOR_BGR2GRAY
            )
            ...
            _, mask_triangles_designed = cv2.threshold(
                img_morphing_rect_area_gray, 1, 255, cv2.THRESH_BINARY_INV
            )
            ...
            img_morphing[y : y + h, x : x + w] = cv2.add(
                img_morphing_rect_area, warped_triangle
            )
