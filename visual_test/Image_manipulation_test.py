...
def show_test() :
    ...
    img_color = Image.get_face("Test to show an image in color and in black and white")
...
def draw_circle_test() :
    ...
    img = Image.get_face("Test of the draw of two circles (one is filled and not the other one)")
...
def detect_faces_test_no_face() :
    ...
    img = get_test_no_face("Test to show that there is no glitch when trying to detect a face when there is not")
...
def morphing_test(debug : bool = False) :
    ...
    img2 = Image.get_face("Test to show the morphing: second face", model=False)
